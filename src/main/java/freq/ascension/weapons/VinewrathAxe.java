 ...
 *
 * <p>
 * A retextured Diamond Axe enchanted with Sharpness V, Efficiency V, and Curse
 * of Vanishing.
 *
 * <p>
 * <b>Active ability (onAttack):</b> When the Flora god strikes a blocking
 * player, the hit
 * disables their shield (axes always break shields). One tick later this
 * weapon:
 * <ol>
 * <li>Extends the shield cooldown from the vanilla 100 ticks to 200 ticks (10
 * seconds).</li>
 * <li>Applies vine freeze — Slowness 255 for 3 seconds (60 ticks) with the
 * thorns
...
public class VinewrathAxe implements MythicWeapon {
    ...
    public ItemStack createItem() {
        ...
        stack.set(DataComponents.LORE, new ItemLore(List.of(
                Component.literal("Hitting a blocking player extends their shield cooldown")
                        .withStyle(s -> s.withItalic(true).withColor(ChatFormatting.GRAY)),
                Component.literal("to 10 seconds and applies vine freeze (3s stun).")
                        .withStyle(s -> s.withItalic(true).withColor(ChatFormatting.GRAY)),
                Component.literal("All hits deal 15% of target's max HP as spell damage.")
                        .withStyle(s -> s.withItalic(true).withColor(ChatFormatting.GRAY)))));
     ...
     *
     * <p>
     * Axes in vanilla always disable a blocking player's shield, applying a
     * 100-tick cooldown.
     * We capture the blocking state before a 1-tick delay (by tick+1, isBlocking()
     * may have
     * already changed), then override the cooldown and layer vine freeze + spell
     * damage.
     *
     * <p>
     * Vine freeze (Slowness 255 for 60 ticks) is applied to blocking victims
     * regardless of
    ...
    public void onAttack(ServerPlayer attacker, LivingEntity victim, Order.DamageContext ctx) {
        ...
        Ascension.scheduler.schedule(new DelayedTask(1, () -> {
            ...
            // This applies whether or not they were blocking — the axe always disables
            // shield
            ...
            if (wasBlocking) {
                ...
                victimPlayer.addEffect(
                        new MobEffectInstance(MobEffects.SLOWNESS, Config.vinewrathAxeStunTicks, 255, false, false));
                victimPlayer.setDeltaMovement(0, 0, 0);
                victimPlayer.setNoGravity(true);

                // Zero out velocity every tick for the full stun duration so no knockback
                // applies
                int[] stunTicks = { 0 };
